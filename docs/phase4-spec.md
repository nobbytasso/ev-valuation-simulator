...
6件の恒常 span=0 ドライバーの扱いは金額表示の意味に関わるため、未確定事項 P4-1 として裁定を求める(推奨: SENSITIVITY_DRIVERS から削除)。
...
| C2 | `feat(engine): 構造的span=0ドライバーの整理`(**P4-1裁定が「削除」の場合のみ**) | SaaS/メディア/EC の SENSITIVITY_DRIVERS から6件削除+各テスト修正+engine-spec §1.5 同時更新(§7.2) | golden 差分なし(SENSITIVITY_DRIVERS は golden 非対象) |
...
| C4 | `feat(ui): トルネードチャートセクション` | §3 SensitivitySection+6ビューへの組み込み。テスト: 上位N制限とトグル・感度なしグループ・δ変更で再計算・ドメイン外入力でエラー表示 | — |
...
### 7.2 §1.5 感度分析への追記(C2 と同時。P4-1 裁定が「削除」の場合)
...
P4-1 裁定が「残置」の場合は、削除の段落に代えて「上記6ドライバーは恒常的に span=0 となる(UIは『感度なし』として表示する)」の注記を入れる。

### 7.3 §4 未確定事項表への追記

> | U-21 | 感度分析: EVに影響しないドライバーの扱い | P4-1 の裁定を転記 | 裁定後に確定 |
> | U-22 | 資本政策: Exit年の持ち方(vcMethod.yearsToExit 共用) | P4-5 の裁定を転記 | 裁定後に確定 |

---

## 8. 未確定事項一覧(ユーザー裁定待ち)

実装セッションはこの表の裁定結果を受領してから着手する。**推奨のまま進めてよい場合も、その旨の明示的な裁定を要する。**
...
> 以下は Phase 4-B(実装)セッション冒頭に貼るドラフト。《裁定》の箇所はユーザーの P4-1〜P4-7 裁定結果で置換してから使用する。
...
- P4-1: 《裁定》 / P4-2: 《裁定》 / P4-3: 《裁定》 / P4-4: 《裁定》 / P4-5: 《裁定》 / P4-6: 《裁定》 / P4-7: 《裁定》

### 実装順序とコミット

phase4-spec.md §6 の C1〜C9 の順に、1コミットずつ進めること。**最初のコミットは必ず C1(創薬 buildTornado 統合テスト。監査ゲート条件3)**。P4-1 が「残置」の場合 C2 はスキップし、§7.2 の残置版注記のみ C3 で engine-spec に反映する。
