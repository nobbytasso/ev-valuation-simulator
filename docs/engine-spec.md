...
**Exit時**: 未消化プールは**発行済みとして扱い、按分の分母に残す**(投資家の手取りが小さくなる保守的仮定 → U-14)。優先分配権(liquidation preference)は v1 では扱わない(→ U-13)。

```
ファンド手取り = ファンドの最終持分 × exit.equityValue
...
実装は各項の「仮採用」で進め、コードに `TODO(U-n)` を付す。**金額計算に影響する U-1, U-4, U-6, U-10, U-11, U-14 は特に確認が必要。**

| # | 論点 | 仮採用 |
| - | ---- | ------ |
| U-1 | SaaSマルチプルのARR基準(実績 vs NTM) | NTM(`arrBasis` 入力で切替可) |
| U-2 | SaaS DCFのFCFマージン改善パス | 一定マージン(改善パスは将来拡張) |
| U-3 | NRR/チャーン/CAC回収を評価式に組み込むか | 組み込まない(ベンチマーク対比指標のみ) |
| U-4 | 創薬のレンジ生成方法 | 割引率3点(12%/11%/10%) |
| U-5 | 創薬: launchYear入力とフェーズ期間合計の不整合 | launchYear を正とし、不整合は警告表示のみ |
| U-6 | 創薬の評価ホライズン | 上市後15年で打ち切り |
| U-7 | 医療機器の浸透カーブ形状 | 線形ランプ(ロジスティックは将来拡張) |
| U-8 | 医療機器: チャネルコスト・償還価格の分離 | operatingMargin に内包(償還下振れは価格レバーで表現) |
| U-9 | メディア: MAU成長の構造化(獲得−解約分解) | 成長率直接入力(churnは診断のみ) |
| U-10 | EC: LTVの定義(F2転換率→年次リピート率の近似、上限年数) | 近似採用、上限10年 |
| U-11 | クライメート: 非オフテイク販売分の実現率 | 1.0(割引なし)、入力で変更可 |
| U-12 | ネットデット(EV→株式価値変換) | `netDebt` 入力、既定0 |
| U-13 | 希薄化: 優先分配権(liquidation preference) | v1 スコープ外(単純按分) |
| U-14 | 希薄化: Exit時の未消化プールの扱い | 発行済み扱い(保守的) |
| U-15 | 感度分析: 定義域外クランプ vs 加法変動 | 相対±20% + 定義域クランプ |
| U-16 | 期央割引(mid-year convention) | 期末割引のみ |
| U-17 | シナリオプリセットの具体値(全セクター) | Phase 3 で確定。仮値+TODO |
| U-18 | 創薬フェーズ成功確率の既定値と出典 | 仮値同梱(出典確認は Cowork/文献レビューで) |
| U-19 | マルチプル・割引率の既定値 | ダミー値。`benchmarks.dummy.json` と整合させる |
