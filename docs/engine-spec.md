...
/**
 * `Cashflow.t`(§1.1 PV/NPV、§1.3 IRR/MOIC)は年単位の**非負の実数**を許容する
 * (年フラクション。例: 投資日から評価日までの 2.37 年)【Phase 5追記】。
 * `presentValue` / `irrBisection` は `Math.pow(1+r, t)` によりそのまま成立するため
 * 実装変更は不要(回帰テストのみ追加)。`YearIndex`(整数)はセクターモデルの年次CF・
 * ラウンド年で従来どおり使用し、実数年はポートフォリオの未実現IRR等、
 * セクターモデル外の呼び出し側でのみ用いる。
...
エンジンは以下のドメイン外入力に `ok: false`(`code: "OUT_OF_DOMAIN"`、複数違反は全件列挙)を返す。※印は Phase 3 時点で実装済み、無印は D-2 バッチで実装する。UI フォームにも対応する min/max 属性を付与する。

**共通ルール**

- 確率の意味を持つ `Ratio`: [0, 1]
- マルチプル(`Range3<number>`): 各値 > 0
- 割引率(`Range3<Ratio>` / 単一): 各値 > 0。Gordon TV を使うモデルは r > terminalGrowth ※(SaaS・医療機器で実装済み)
- 金額(`Money` / `Yen`): ≥ 0(明記あるものを除く)
- 年数・期間(`projectionYears` 等): ≥ 1 の整数(`plateauYears`・`approvalDelayYears` は ≥ 0)
...
| --- | --- | --- | --- |
| SaaS | arrGrowth | > −1 | |
| SaaS | operatingMargin / fcfMargin | [−1, 1] | 負のマージン許容 |
| SaaS | growthDecayFactor | (0, 1] | |
| 創薬 | phaseSuccessProbs(各) | [0, 1] | 0/1 は有効な境界値(golden検証済み) |
| 創薬 | phaseDurations(各) | ≥ 1 の整数 | 開発費の年割り分母 |
| 創薬 | declineRate | [0, 1] | |
| 医療機器 | procedureGrowth | > −1 | |
| 医療機器 | recurringRatio | [0, 1) ※ | 1 で売上発散(実装済み) |
| メディア | mauGrowth | > −1 | |
| メディア | monthlyChurn / contentCostRatio / dauMauRatio | [0, 1] | |
| メディア | growthDecayFactor | (0, 1] | |
| EC/D2C | **f2Rate** | **[0, 1)** | 1 以上は ok:false(1 未満なら lifetimeYears は maxLifetimeYears でキャップ) |
| EC/D2C | revenueGrowth | > −1 | |
| EC/D2C | grossMargin / adCostRatio / logisticsCostRatio | [0, 1] | |
| EC/D2C | inventoryTurnover | > 0 | 対比指標 |
| クライメート | costDeclineRate | [0, 1) | 1 でコスト0固定、超は符号反転のため不可 |
| クライメート | subsidyCoverage / massProductionProb / offtakeCoverage / merchantRealization | [0, 1] | |
| クライメート | rampYears | ≥ 1 の整数 | 0 はゼロ除算 |

**発散しうる派生指標の挙動定義(バリデーションエラーではなくキー省略)**

- メディア: `arpuTotal × (1 − contentCostRatio) = 0` かつ `monthlyChurn > 0` のとき、`paybackMonths` は分母0で計算不能 → **キー省略**(§0.2 のルール適用)。`ltv` は 0 として出力する。`ltvCpaRatio` も LTV=0 として出力してよい(0/cpa = 0)。
- メディア: `monthlyChurn = 0` のとき LTV系(avgLifetimeMonths / ltv / ltvCpaRatio / paybackMonths)全てキー省略(既存仕様)。
...
- **時点 `t_i` は非負の実数を許容する**(§0.1 に定義域拡張を明記。ポートフォリオの未実現IRR等、投資日→評価基準日が小数年になる呼び出しで使用。実装は変更なし)。
...
**Exit年の持ち方(Phase 4確定、U-22)**: `exit.yearIndex` は資本政策シミュレーター(UI層)では `vcMethod.yearsToExit` を共用し、独自フィールドを持たない。含意IRR(VC法)と期待IRR(資本政策)が異なるExit時点で計算される不整合を避けるため。
...
**paybackMonths のガード条件【v0.4明確化、D-3】**: `paybackMonths` は `monthlyChurn > 0` であれば **`cpa = 0` でも出力する**(値は 0)。`cpa > 0` ガードが掛かるのは `ltvCpaRatio` のみ。ただし分母 `ARPU_total × (1 − contentCostRatio) = 0` のときは計算不能としてキー省略(§0.2.1)。
※ Phase 3 レビュー(docs/review-phase3.md B-4)で、Python 参照実装が誤って `cpa > 0` で `paybackMonths` をガードしていたことが判明(TS 実装が本仕様準拠)。D-3 バッチで Python 側を修正し、golden に境界ケース(`cpa = 0` かつ `monthlyChurn > 0`)を追加すること。
...
| U-22 | 資本政策: Exit年の持ち方 | vcMethod.yearsToExit を共用(capitalPolicyに独自フィールドを持たない。Phase 4, P4-5裁定) | **確定** |
