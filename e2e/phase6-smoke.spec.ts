...
test('C8-1. 円形ゲージ(EVベース・Rule of 40・含意IRR)が結果・VC法セクションに描画される(§4.2)', async ({ page }) => {
  await createScenario(page, 'SaaS(日本)')
  const gauges = page.locator('.circular-gauge')
  await expect(gauges).toHaveCount(3) // EVベース・Rule of 40・VC法含意IRR
  await expect(page.getByRole('img', { name: /企業価値\(ベース\):/ })).toBeVisible()
  await expect(page.getByRole('img', { name: /Rule of 40:/ })).toBeVisible()
  await expect(page.getByRole('img', { name: /含意IRR:/ })).toBeVisible()
})

test('C8-2. 資本政策の期待IRR/MOICは自ファンド出資がなければゲージを出さず「—(理由)」のまま(§4.2)', async ({ page }) => {
  await createScenario(page, 'SaaS(日本)')
  await expect(page.getByText('—(自ファンドの出資がありません)').first()).toBeVisible()
  const capitalPolicyGauges = page.locator('.capital-policy-section__gauges .circular-gauge')
  await expect(capitalPolicyGauges).toHaveCount(0)
})

test('C9-1. プリセット適用時に結果パネルへスキャン走査クラスが一時的に付与される(ダーク、P6-8)', async ({ page }) => {
  await createScenario(page, 'SaaS(日本)')
  const resultSection = page.locator('.sector-scenario-view > section', { hasText: '結果' }).first()
  await expect(resultSection).not.toHaveClass(/scan-active/)

  await page.locator('.sector-scenario-view__presets button').first().click()
  await expect(resultSection).toHaveClass(/scan-active/)
  await expect(resultSection).not.toHaveClass(/scan-active/, { timeout: 2000 })
})

test('C9-2. prefers-reduced-motionではスキャン走査クラスが付与されない', async ({ page }) => {
...
test('C9-3. 日英併記キャプションは両テーマでDOMに常設される(P6-6)', async ({ page }) => {
  await createScenario(page, 'SaaS(日本)')
  const caption = page.locator('.section-heading__caption-en', { hasText: 'ENTERPRISE VALUE' })
...
test('C10-1. ライトでプリセット適用時にパーティクル(ハート/スパークル)が発火する(§6.2)', async ({ page }) => {
  await createScenario(page, 'SaaS(日本)')
  await page.getByRole('button', { name: 'テーマ切替' }).click() // ライトへ切替

  await page.locator('.sector-scenario-view__presets button').first().click()
  await expect(page.locator('.particle-burst-layer .particle-burst__item').first()).toBeVisible()
  await expect(page.locator('.particle-burst-layer .particle-burst').first()).toHaveCount(0, { timeout: 2000 })
})

test('C10-2. ダークではパーティクル層が非表示のまま(§6.2はライト専用)', async ({ page }) => {
  await createScenario(page, 'SaaS(日本)') // 既定=ダーク
  await page.locator('.sector-scenario-view__presets button').first().click()
  await expect(page.locator('.particle-burst-layer')).toBeAttached()
  await expect(page.locator('.particle-burst-layer')).not.toBeVisible()
})

test('C10-3. prefers-reduced-motionではパーティクルが発火しない', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await createScenario(page, 'SaaS(日本)')
  await page.getByRole('button', { name: 'テーマ切替' }).click() // ライトへ切替
  ...
  await page.locator('.sector-scenario-view__presets button').first().click()
  await page.waitForTimeout(200)
  await expect(page.locator('.particle-burst-layer .particle-burst')).toHaveCount(0)
})

test('C10-4. ボタンにpunni-scaleトークンが適用される(§6.2、値そのものはCSS変数で理論値確認)', async ({ page }) => {
  await createScenario(page, 'SaaS(日本)')
  const darkScale = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--button-active-scale').trim())
  expect(darkScale).toBe('0.98')

  await page.getByRole('button', { name: 'テーマ切替' }).click()
  const lightScale = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--button-active-scale').trim())
  expect(lightScale).toBe('0.92')
})

test('C11-1. prefers-reduced-motionではゲージのカウントアップが発火せず確定値を即時表示する(§8.2)', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await createScenario(page, 'SaaS(日本)')
  // カウントアップ演出が有効なら初期フレームでは0近辺から始まるが、reduced-motion時は
  // useCountUpが即座にtargetへ設定するため、描画直後から確定値が見える。
  await expect(page.getByRole('img', { name: /企業価値\(ベース\): 10,400 百万円/ })).toBeVisible()
})

test('C11-2. 単位トグルはチャート軸ラベルにも連動する(億円、§8.2)', async ({ page }) => {
  await createScenario(page, 'SaaS(日本)')
  await page.getByRole('button', { name: '金額単位切替' }).click()
  const cashflowChart = page.locator('.cashflow-chart')
  await expect(cashflowChart.getByText('(億円)')).toBeVisible()
})
...
test('C2-1. テーマ切替でレイアウトシフトが起きない(主要要素のboundingBoxが±1px)', async ({ page, context }) => {
  ...
  // またダークのheading-decode演出(letter-spacingを含む、§6.1)がmotion-duration-slow(600ms)で
  // 完了するまでは見出し幅が変動しうるため、これも完全に収まってから計測する(既存E2Eの
  // 待機パターン踏襲、出典: e2e/phase4-smoke.spec.ts)。
